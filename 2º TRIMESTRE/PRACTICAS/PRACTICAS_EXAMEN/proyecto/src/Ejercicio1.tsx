...
function Ejercicio1(){
    ...
    const elementosFiltrados = items.filter(
        (item) => item.toLowerCase().includes(terminoBuscado.toLowerCase())
    );
        ...
        <div className="mx-auto p-4 max-w-md">
            ...
            <div className="mb-4">
                <input type="text" 
                    ...
                    className="p-2 border border-blue-300 rounded-lg focus:outline-none w-full"
                />
            </div>
            <ul className="space-y-2">
                {elementosFiltrados.length > 0 ? (
                    elementosFiltrados.map((item, index) => (
                        <li
                            key={index}
                            className="bg-gray-100 hover:bg-cyan-200 shadow-sm p-2 rounded-lg text-gray-700"
                        >
                            {item}
                        </li>
                    ))
